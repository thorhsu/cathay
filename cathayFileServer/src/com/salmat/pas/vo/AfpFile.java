...
@NamedQueries({
	...
	@NamedQuery(name="AfpFile.findNotFeedBack", query="from AfpFile where (feedback is null or feedback = false) and newBatchName like 'CA%'"),
	@NamedQuery(name="AfpFile.findMaxSerialNo", query="select max(centerSerialNo) from AfpFile where insertDate > ? and center = ? and batchOrOnline = ?"),
	@NamedQuery(name="AfpFile.findMaxReceiptSerialNo", query="select max(centerSerialNo) from AfpFile where insertDate > ? and center = ? and batchOrOnline = ? and receipt = true"),
	@NamedQuery(name="AfpFile.findMaxBatNo", query="select max(serialNo) from AfpFile")
