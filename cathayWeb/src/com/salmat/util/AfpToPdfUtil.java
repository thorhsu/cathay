...
public class AfpToPdfUtil {
	...
	private static String afpToPdfDir  = "";
	private static long waitConvertTime = 2 * 60 * 60 * 1000; //轉pdf等待時間，設為2小時
	...
	public static boolean convertToPdf(String afpFile,
			String detFile, boolean isStreamEdp) throws IOException,
			InterruptedException {
		if (isStreamEdp)
			return streamEdpToPdf(afpFile, detFile);
		else
			return afpToPdf(afpFile, detFile);
	...
	public static boolean afpToPdf(String afpToPdfDir, String afpFile, String detFile)
			throws IOException, InterruptedException {
		/*
        ...
        */
		...
		while(!afp2PdfRun.isFinishProcess()){
			...
			long nowTime = new Date().getTime();
			logger.info("waiting Afp2Pdf convert " + afpFile + "  to " + detFile + ". Waiting time " + (nowTime - beginProcess) / 1000 + " seconds.");
			//如果超過等待時間，就強迫跳出
			if((nowTime - beginProcess) >  waitConvertTime){
				logger.warn(afpToPdfDir + "afp2pdf.exe -o " + detFile + " "
